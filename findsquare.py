...
def find_squares(img_org):
    ...
    for cnt in contours:
        ...
        if len(cnt) == 4 and cv.contourArea(cnt) > 5600  and cv.contourArea(cnt) < 5800 and cv.isContourConvex(cnt): #and cv.contourArea(cnt) < 5000  len(cnt) == 4 and   and cv.isContourConvex(cnt)
            #print("---------------------------------------------------------------------------")
            ...
            crop_img = img_org[y:y+h+1, x+1:x+1+w]
            find = True
            # 只检测矩形（cos90° = 0）
            #if max_cos < 0.1:
            # 检测四边形（不限定角度范围）
